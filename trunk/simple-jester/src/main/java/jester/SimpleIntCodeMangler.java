...
public class SimpleIntCodeMangler extends SimpleCodeMangler {
	...
	int indexOfNumber(IgnoreListDocument s) {
		...
		for (int index = startIndex; index < length; index++) {
            ...
            if (characterToCheck == '0') { // don't mangle zero that starts hex constant
                if (index < length-1) {
                    char nextCharacter = s.charAt(index+1);
                    if (nextCharacter == 'x') continue;
                }
            }
			if (Character.isDigit(characterToCheck)){
				if (index > 0) {
					char precedingCharacter = s.charAt(index - 1);
					boolean ignoreThisBecauseItIsEitherWrongToMutateOrTheNumberHasAlreadyBeenMutated 
                     = Character.isLetterOrDigit(precedingCharacter);
                   
                   if (index > 1) { // do mutate first digit of hexadecimal constant
